...
export async function GET(request: NextRequest) {
  try {
    ...
    const message = error instanceof Error ? error.message : "Nao foi possivel carregar a galeria.";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Nao foi possivel criar item da galeria.";
    return jsonError(message, 500);
  }
}
